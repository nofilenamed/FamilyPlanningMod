...
namespace FamilyPlanning.Patches
{
    ...
    class PickPersonalFarmEventPatch
    {
        public static bool Prefix(ref FarmEvent result)
        {
            ...
            if (isMarried && Game1.player.GetSpouseFriendship().DaysUntilBirthing <= 0 && Game1.player.GetSpouseFriendship().NextBirthingDate != null)
            {
            ...
            if (isMarried)
            {
                ...
                if (npcSpouse != null && Game1.getCharacterFromName(npcSpouse, true).canGetPregnant() && Game1.player.currentLocation == Game1.getLocationFromName(Game1.player.homeLocation.Value))
                {
                    ...
                    bool isGaySpouse = Game1.getCharacterFromName(npcSpouse, true).isAdoptionSpouse();
